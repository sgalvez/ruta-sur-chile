...
import type { Catalog, Place, State, Service } from './types';
import { REGIONS, STATUS_LABELS, filterPlaces, effectiveStatus, distanceKm, safeExternalUrl, isStale } from './domain.mjs';
...
const kindLabels = { camping: 'Camping', park: 'Parque nacional', reserve: 'Reserva nacional' };
let catalog: Catalog;
let state: State = { schemaVersion: 1, attemptedAt: null, lastSuccessfulAt: null, observations: {} };
...
const filters = { query: '', region: '', kind: '', status: '', service: '', saved: false, car: false };
...
function skeleton() {
  $('#app').innerHTML = `
    <header class="site-header"><a href="#" class="brand" aria-label="Ruta Sur Chile, inicio"><span class="brand-mark">${icon('mountain')}</span><span>RUTA SUR <small>CHILE</small></span></a>
      <nav aria-label="Navegación principal"><button class="nav-link active" data-view="explore">Explorar</button><button class="nav-link" data-view="saved">Mis lugares <span class="saved-count">0</span></button><button class="nav-link" id="about">Sobre los datos ${icon('info')}</button></nav>
      <span class="season">${icon('tree')} Primavera 2026</span>
    </header>
    <main>
      <section class="hero" aria-labelledby="hero-title"><div class="hero-copy"><div class="eyebrow"><span></span> DE O’HIGGINS A CHAITÉN</div><h1 id="hero-title">El sur,<br><em>a tu ritmo.</em></h1><p>Encuentra tu próxima parada entre bosques, lagos y caminos que dan ganas de seguir.</p><a class="hero-link" href="#explorar">Explorar campings y parques ${icon('arrow')}</a></div>
        <div class="journey" aria-label="Un viaje por siete regiones"><div class="contours"></div><div class="journey-path"></div><div class="journey-start"><span class="route-dot"></span><small>EL PUNTO DE PARTIDA</small><strong>O’Higgins</strong></div><div class="journey-middle">${icon('tree')} Bosques, lagos<br>y una buena carpa.</div><div class="journey-end"><span class="route-dot"></span><small>RUMBO A LA PATAGONIA</small><strong>Chaitén</strong></div><div class="journey-tag">${icon('tent')} La mejor ruta es la tuya.</div></div>
      </section>
      <section class="trip-strip" aria-label="Información del catálogo"><div><strong id="total-count">—</strong><span>lugares para descubrir</span></div><div><strong>07</strong><span>regiones en el camino</span></div><div class="strip-status">${icon('clock')}<span id="freshness">Consultando las fuentes…</span><button id="reload" class="icon-button" aria-label="Actualizar catálogo">↻</button></div><button id="offline" class="offline-button">${icon('download')}<span>Preparar para el viaje</span></button></section>
      <section class="explore" id="explorar" aria-labelledby="explore-title"><div class="section-heading"><div><span class="eyebrow">TU PRÓXIMA PARADA</span><h2 id="explore-title">Hay un lugar para ti.</h2></div><div class="type-tabs" role="group" aria-label="Tipo de lugar"><button data-kind="" class="active">Todos</button><button data-kind="camping">${icon('tent')} Campings</button><button data-kind="nature">${icon('tree')} Parques y reservas</button></div></div>
    ...
    <nav class="mobile-nav" aria-label="Vistas"><button data-view="explore" class="active">${icon('list')}Explorar</button><button data-view="map">${icon('map')}Mapa</button><button data-view="saved">${icon('heart')}Guardados <span class="saved-count">0</span></button></nav>
...
function bindEvents() {
  ...
  document.querySelectorAll<HTMLElement>('[data-kind]').forEach(b=>b.addEventListener('click',()=>{filters.kind=b.dataset.kind!;document.querySelectorAll('[data-kind]').forEach(el=>el.classList.toggle('active',el===b));render();}));
  $('#clear').addEventListener('click',()=>{Object.assign(filters,{query:'',region:'',kind:'',status:'',service:'',car:false});for(const id of ['query','region','service','status']) $<HTMLInputElement>('#'+id).value='';$<HTMLInputElement>('#car').checked=false;document.querySelectorAll<HTMLElement>('[data-kind]').forEach(el=>el.classList.toggle('active',el.dataset.kind===''));render();});
...
function render() {
  ...
  $('#clear').hidden=!Object.entries(filters).some(([k,v])=>k!=='saved'&&Boolean(v));
...
function renderMarkers() {
  ...
  layer.clearLayers();
  for(const p of visiblePlaces){const marker=L.marker(p.coordinates,{icon:L.divIcon({className:'place-marker '+p.kind,html:icon(p.kind==='camping'?'tent':'tree'),iconSize:[34,34],iconAnchor:[17,17]}),title:p.name,alt:p.name});marker.bindTooltip(esc(p.name),{direction:'top',offset:[0,-16]});marker.on('click',()=>openDetail(p.id));marker.addTo(layer);}
...
async function fetchJson(path: string) {
  ...
  if(!response.ok)throw new Error('Respuesta inválida');return response.json();
}
async function loadData() {
  if(!navigator.onLine){
    try{
      const savedCatalog=JSON.parse(localStorage.getItem('ruta-sur-catalog')||'null');
      const savedState=JSON.parse(localStorage.getItem('ruta-sur-state')||'null');
      if(savedCatalog?.schemaVersion===1&&savedCatalog.places?.length){catalog=savedCatalog;if(savedState?.schemaVersion===1&&savedState.observations)state=savedState;return;}
    }catch{/* The service worker cache is the fallback when storage is unavailable. */}
  }
  let freshCatalog: Catalog;
  try {
    freshCatalog=await fetchJson('./data/catalog.json');
    if(freshCatalog.schemaVersion!==1||!Array.isArray(freshCatalog.places)||!freshCatalog.places.length)throw new Error('Catálogo inválido');
    try{localStorage.setItem('ruta-sur-catalog',JSON.stringify(freshCatalog));}catch{/* Service worker is the main offline store. */}
  }catch(error){const stored=localStorage.getItem('ruta-sur-catalog');if(!stored)throw error;freshCatalog=JSON.parse(stored);}
  catalog=freshCatalog;
  try{const next=await fetchJson('./data/state.json');if(next.schemaVersion!==1||!next.observations)throw new Error('Estado inválido');state=next;try{localStorage.setItem('ruta-sur-state',JSON.stringify(state));}catch{}}
  catch{try{const stored=localStorage.getItem('ruta-sur-state');if(stored)state=JSON.parse(stored);}catch{}}
}
async function reload(feedback=true) {
  $<HTMLButtonElement>('#reload').disabled=true;
  try{await loadData();render();if(activePlace)renderDetail(activePlace);if(feedback)toast(navigator.onLine?'Catálogo cargado. Revisa las fechas de consulta de cada fuente.':'Estás usando el catálogo guardado.');}
  catch{if(feedback)toast('No se pudo actualizar. Conservamos los datos que ya tenías.');}
  finally{$<HTMLButtonElement>('#reload').disabled=false;}
}
async function prepareOffline(feedback=false) {
  ...
  try {
    ...
    if(offlineReady){$('#offline').innerHTML=icon('check')+'<span>Fichas listas sin señal</span>';$('#offline').classList.add('ready');if(feedback)toast('Las fichas y tus favoritos están disponibles sin señal. El mapa necesita conexión.');}
